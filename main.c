...
Premiere étape du pivot de gauss : mettre un 1 de la première colonne en position Matrice[0][0]
*/
int SwapLine(int Line) {
	int flag, lineswap, i, auxb; //variable de fonctions
	flag = 0; 
	int aux[25]; //création d'un tableau auxiliaire
...
void Multiple(int LL, int nombre) {
	...
	sln = pow(2, nombre);
	for (n = 0; n < sln; n++) {
		//Sauvegarde de la matrice générée et du tableau initial car il faut les régénérer a chaque calcul de solution
		for(i = 0; i < nombre; i++)
			for(j = 0; j < N; j++){
